...
import (
	...
	"github.com/yohannes/kidasie-backend/internal/slidecontent"
)

func main() {
	if err := run(); err != nil {
		slog.Error("slide conversion failed", "error", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	flag.StringVar(&sourcePath, "file", "", "path to corrected slide JSON")
	...
	flag.StringVar(&sectionTitle, "section-title", "", "English section title")
	flag.StringVar(
		&sectionTitleAm,
		"section-title-am",
		"",
	...
	if err != nil {
		return fmt.Errorf("open slide file: %w", err)
	}
	defer sourceFile.Close()

	source, err := slidecontent.Decode(sourceFile)
	if err != nil {
		return err
	}

	document, stats, err := slidecontent.Convert(
		source,
...
func writeJSONAtomically(path string, value any) error {
	...
	tempFile, err := os.CreateTemp(directory, ".content-*.json")
	if err != nil {
